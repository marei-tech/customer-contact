...
SYSTEM_PROMPT_EMPLOYEE_SELECTION = """
    ...
    {format_instruction}
"""
# 問題2：担当者選定理由のプロンプトテンプレートを定義
SYSTEM_PROMPT_SELECTION_REASON = """
    # 命令
    以下の「選定された従業員情報」を基に、それぞれの従業員を担当者として選定した理由を生成してください。
    ...
    # 生成条件
    - 各従業員について、名前、部署、役職を含めた自然な文章形式で選定理由を記述してください。
    - 例：「棚橋由香里さんは、技術部の課長として、過去に同様の問い合わせに対応した経験があり、品質管理とテスト実施に関する専門知識を持っています。」
    - 選定理由には、過去の対応履歴、得意分野、主要業務、保有資格など、具体的な根拠を含めてください。
    - 各従業員の選定理由は、改行で区切って記述してください。
    - 簡潔かつ具体的に記述してください。
"""
